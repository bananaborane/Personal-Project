...
export class Bike extends Component {
  ...
  render() {
    ...
    let { user_id, bike_id, title, description, image_url, bike_size, wheel_size, city, unitedState, username } = this.props;
    return (
      <Link to={{
        ...
        <div className='each-bike'>  
            ...
            <div className='bike-right-column'>
                ...
                <div>{ isItTrue ? (<button onClick={()=>{}}>Remove your bike from marketplace</button>) : (null) }</div>
            </div>
        </div>
      </Link>
    )
